...
def layer(previous_layer_outputs, this_weights, this_biases, neuron_count):
	layer_outputs = []
	for i in range(neuron_count):
		output = neuron(previous_layer_outputs, this_weights[i], this_biases[i])
...
def trainnn(training_data, weight_matrix, bias_matrix, weights2, biases2):
	...
	for i in range(len(training_data)):
		output = layer(training_data[i].flatten(), weight_matrix, bias_matrix, 16)
		trained_data.append(output)

	# Output Layer
	for i in range(len(trained_data)):
		output = layer(trained_data[i], weights2, biases2, 10)
		output_data.append(output)

	return trained_data, output_data
...
def generateHiddenError(error, hidden_outputs):
	...
	for output_error, hidden_output in zip(error, hidden_outputs):
		hidden_error = np.dot(output_error, weight_matrix2.T) * sigmoidderivative(np.array(hidden_output))
...
if __name__ == "__main__":
	...
	weight_matrix = np.random.rand(16, 784) # Initialise first layer matrices with random values between 0 and 1.
	# Weight Matrix
	...
	weight_matrix2 = np.random.rand(10, 16)
	bias_matrix2 = np.random.rand(10,)
	
	for epoch in range(epochs):
	# Forward Pass
		hidden_outputs, trained_data = trainnn(train_X, train_y, weight_matrix, bias_matrix, weight_matrix2, bias_matrix2)
		print("Forward pass complete.")

		cost = generateC(trained_data, train_y)
		error = generateError(trained_data, train_y)
		hidden_error = generateHiddenError(error, hidden_outputs)
	...
	hidden_test_outputs, test_predictions = trainnn(test_X, test_y, weight_matrix, bias_matrix, weight_matrix2, bias_matrix2)
	correct = 0
	for i in range(len(test_y)):
